...
class OAuth2Validator(RequestValidator):
    ...
    def _get_token_from_authentication_server(
        ...
        if "active" in content and content["active"] is True:
            ...
            max_caching_time = datetime.now() + timedelta(
                seconds=auth_settings.RESOURCE_SERVER_TOKEN_CACHING_SECONDS
            )
    ...
    def validate_bearer_token(self, token, scopes, request):
        ...
        introspection_url = auth_settings.RESOURCE_SERVER_INTROSPECTION_URL
        introspection_token = auth_settings.RESOURCE_SERVER_AUTH_TOKEN
        introspection_credentials = auth_settings.RESOURCE_SERVER_INTROSPECTION_CREDENTIALS
    ...
    def is_pkce_required(self, client_id, request):
        ...
        """
        if callable(auth_settings.PKCE_REQUIRED):
            return auth_settings.PKCE_REQUIRED(client_id)
        return auth_settings.PKCE_REQUIRED
    ...
    def rotate_refresh_token(self, request):
        ...
        """
        return auth_settings.ROTATE_REFRESH_TOKEN
    ...
    def save_bearer_token(self, token, request, *args, **kwargs):
        ...
        expires = timezone.now() + timedelta(
            seconds=token.get(
                "expires_in",
                auth_settings.ACCESS_TOKEN_EXPIRE_SECONDS,
            )
        )
    ...
    def validate_refresh_token(self, refresh_token, client, request, *args, **kwargs):
        ...
        null_or_recent = Q(revoked__isnull=True) | Q(
            revoked__gt=timezone.now() - timedelta(seconds=auth_settings.REFRESH_TOKEN_GRACE_PERIOD_SECONDS)
        )
        rt = (
